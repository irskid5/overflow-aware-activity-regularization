...
**Architecture:** Create a single run directory per four-step experiment, with step subdirectories. Pass `run_dir` and `step` from orchestrator to train function. Save configs as JSON. Capture experiment-level logs (no per-step logs to avoid duplication).

**Tech Stack:** Python 3.10, TensorFlow 2.10.1, JSON for configs
...
Add helper functions to `experiments/mnist/training.py` (after `create_run_dir`):
...
Then add this call in `train()`, right after `os.makedirs(step_dir, exist_ok=True)`:
...
## Task 3: Extract make_oar_config as module-level helper (DRY fix)

**Files:**
- Modify: `experiments/mnist/config.py`
- Test: `tests/test_mnist_config.py`

### Step 1: Write test for the helper function
...
Run: `PYTHONPATH=. pytest tests/test_mnist_config.py::test_make_oar_config_returns_expected_structure -v`

Expected: FAIL with `ImportError: cannot import name '_make_oar_config'`

### Step 3: Extract the helper and refactor
...
def get_model_parameter_stats(pretrained_weights: str | None, options: dict, layer_options: dict) -> dict:
...
def perform_step_in_four_step_quant(step: int, pretrained_weights: str | None, options: dict) -> str:
    ...
    # Get base layer options for step 4 parameter stats
    base_layer_options = get_default_layer_options(options)
    ...
    if step == 4:
        ...
        tern_params = get_model_parameter_stats(
            pretrained_weights, options, base_layer_options
        )
    ...
    # Build layer options using the extracted helper
    omega = options["oar"]["omega"]
    oar_lambda = options["oar"]["oar_lambda"]
...
git commit -m "refactor(config): extract _make_oar_config helper (DRY)"
```
...
Update the function signature and train() call in `experiments/mnist/config.py`:
...
def perform_step_in_four_step_quant(
    ...
    Returns:
        Path to checkpoints folder of the trained model
    """
    # ... existing code stays the same until the return statement ...
...
Add imports and helper to `experiments/mnist/config.py`:
...
Then update `perform_four_step_quant`:
...
### Step 3: Add tee_output to training.py

Add this context manager to `experiments/mnist/training.py` (after `create_run_dir`):
...
### Step 4: Update perform_four_step_quant to use tee_output
...
```bash
git add experiments/mnist/training.py experiments/mnist/config.py tests/test_mnist_config.py
...
Expected: Creates `runs/mnist/YYYYMMDD-HHMMSS/step_1/` with config.json

### Step 3: Verify directory structure

Run: `find runs/mnist -type f -name "*.json" | head -20`

Expected: Shows config.json files in the new structure
...
4. `perform_four_step_quant()` creates shared run_dir for all steps
5. Experiment-level `config.json` saves initial options
6. Experiment-level `output.log` captures all step outputs (no per-step logs)
7. `_make_oar_config` extracted as module-level helper (DRY fix)
8. `TeeWriter` has `isatty()`, `fileno()`, and `encoding` for Keras compatibility

**Simplifications from Evaluation:**
- Removed per-step `output.log` (redundant with experiment-level log)
- Removed unused `import logging`
- Extracted duplicate `make_oar_config` into `_make_oar_config` helper
- Simplified `_serialize_layer_options` implementation

---

Plan complete and saved to `docs/plans/2026-02-08-phase3.5-organized-outputs.md`. Two execution options:
